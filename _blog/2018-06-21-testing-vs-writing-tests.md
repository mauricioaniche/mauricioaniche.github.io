---
title: "Testing vs writing tests"
layout: page
---

_Testing is different from writing tests. Developers write tests as a a way to give them space to think and confidence for refactoring. Testing focuses on finding bugs. Both should be done._

We are right now at a point where developers *do know* that they need to write automated tests. After all, ideas such as Kent Beck’s [Extreme Programming](https://www.amazon.com/Extreme-Programming-Explained-Embrace-Change/dp/0321278658) and [Test-Driven Development](https://www.amazon.com/Test-Driven-Development-Kent-Beck/dp/0321146530/ref=pd_lpo_sbs_14_t_1?_encoding=UTF8&psc=1&refRID=WE962S6TKV1QRTYEN1WT), Michael Feather's on the [synergy between testing and design](https://vimeo.com/15007792), Steve Freeman's and Nat Pryce's on [how to grow an Object-Oriented software guided by tests](https://www.amazon.de/Growing-Object-Oriented-Software-Addison-Wesley-Signature/dp/0321503627), DHH and Ruby on Rails on [making sure that a web framework comes with a testing framework](http://guides.rubyonrails.org/testing.html), etc, really sticked.

These ideas sticked with me as well. I have been trying to write as many automated tests as I can for the software systems I work since 2007 (I can tell the history of how traumatic my 2006 project was in a later post). I was so excited about Test-Driven Development that I even made it the topic of my [MSc thesis](https://journal-bcs.springeropen.com/articles/10.1186/s13173-015-0034-z). In 2012, I thought I had enough experience with the topic, so I decided to write a [book about how I was practicing TDD](https://www.casadocodigo.com.br/products/livro-tdd), as well as a [book on the basics of tools like JUnit and Selenium](https://www.casadocodigo.com.br/products/livro-testes-de-software) (unfortunately, only available in Brazilian Portuguese).
...
In fact, research has shown quite a few times that doing TDD can improve your class design (see [Janzen](http://digitalcommons.calpoly.edu/cgi/viewcontent.cgi?article=1039&context=csse_fac), [Janzen and Saiedian](http://digitalcommons.calpoly.edu/cgi/viewcontent.cgi?article=1030&context=csse_fac), [George and Williams](https://www.semanticscholar.org/paper/An-Initial-Investigation-of-Test-Driven-Development-George-Williams/66869075d20858a2e9af144b2749a055c6b03177), [Langr](http://eisc.univalle.edu.co/materias/TPS/archivos/articulosPruebas/test_first_design.pdf), [Dogsa and Batic](https://link.springer.com/article/10.1007/s11219-011-9130-2), [Munir, Moyaaed and Petersen](https://pdfs.semanticscholar.org/16d4/2a1eaefb1f404f6da91b12d6c0e710dacb9d.pdf), [myself](https://journal-bcs.springeropen.com/articles/10.1186/s13173-015-0034-z), among others). Recently, [Fucci et al.](https://arxiv.org/pdf/1611.05994.pdf) even argued that the important part is to write tests (and it doesn't matter whether it's before or after). This is also the perception of practitioners, where I quote a [recent blog post from Feathers](http://michaelfeathers.typepad.com/michael_feathers_blog/2008/06/the-flawed-theo.html): *"That’s the magic, and it’s why unit testing works also. When you write unit tests, TDD-style or after your development, you scrutinize, you think, and often you prevent problems without even encountering a test failure."*
...
On the other hand, software testing books from academia, my favorite being [Software Testing Analysis](https://www.amazon.com/Software-Testing-Analysis-Principles-Techniques/dp/0471455938) from Young and Pezzè, have a strong focus on theories and techniques on how to design test cases that explore as much as one can from the program under test. However, with not so many timely examples on how to apply these ideas, which is definitely a requirement for practitioners.
...
* Detecting whether some test suite is able to detect possible bugs. The idea of [mutation testing](https://ieeexplore.ieee.org/abstract/document/5487526/) has [being applied at Google](https://research.google.com/pubs/archive/46584.pdf).

* Automatically generating tests (the famous example is [EvoSuite](https://pdfs.semanticscholar.org/216b/98bb3d9221d5f5d261864975612e4d0faaa6.pdf), which is able to generate tests for Java classes) or to [automatically reproduce crashes](https://pure.tudelft.nl/portal/en/publications/a-guided-genetic-algorithm-for-automated-crash-reproduction(3490acbb-240b-4ec2-8202-712a7d1bb64e).html). I myself worked on [generating tests for SQL queries automatically](https://pure.tudelft.nl/portal/en/publications/searchbased-test-data-generation-for-sql-queries(90a6431f-f78f-4ac3-bf87-c052cd9cd5d4).html).

* Deriving [models from your production system logs](https://pure.tudelft.nl/portal/en/publications/an-experience-report-on-applying-passive-learning-in-a-largescale-payment-company(b463c54a-d69f-4db4-9fcc-cbeb6e2ddf09).html) and build models that can be analysed by domain experts.

* Providing [different code coverage criteria](http://laser.cs.umass.edu/courses/cs521-621/papers/ZhuHallMay.pdf) (you probably have some code coverage tool in your CI) and understand the benefits of each of them.

* Several tools for testing web applications, like [automatically exploring the web app looking for crashes](http://search.proquest.com/openview/c7cf9aff7640da5036f7653ca6a9bfe1/1?pq-origsite=gscholar&cbl=21418&casa_token=Zr4Nz0ZAzfsAAAAA:UTguZ-3zSx1MHQUGirDsM_wejL2UtQRJd93oADSQ2TMCXdTSpAcQveL-YzvQrCtoHhBBn5ywyr0) as well as [finding crashes in REST APIs](https://github.com/EMResearch/EvoMaster/blob/master/docs/publications/2017_qrs.pdf). More can be found in this [web testing literature review](https://www.sciencedirect.com/science/article/pii/S0164121214000223).

* Empirically measuring [how much developers test](https://pure.tudelft.nl/portal/en/publications/developer-testing-in-the-ide-patterns-beliefs-and-behavior(a63e79e0-e0e4-48cd-81ed-87f163810538).html), whether [test smells are important](http://www.cs.loyola.edu/~binkley/papers/icsm-12-smells.pdf) or whether [they are the cause of flaky tests](https://pure.tudelft.nl/portal/en/publications/does-refactoring-of-test-smells-induce-fixing-flaky-tests(c68e527c-bf4a-41f4-827a-6fcab9582c71).html), [why and how mocks are used in Java systems](https://pure.tudelft.nl/portal/en/publications/to-mock-or-not-to-mock(a0b02521-ad00-4f96-9d96-17a85dc23f99).html), and how [developers review test code during code reviews](https://pure.tudelft.nl/portal/en/publications/when-testing-meets-code-review-why-and-how-developers-review-tests(256e7d56-352f-44ae-919b-97fad0eafe69).html).

* Property-based testing is also getting to practitioners by means of tools like [ScalaCheck](https://www.scalacheck.org/) and [QuickTheories](https://github.com/ncredinburgh/QuickTheories).

* Among other things. And if you want to join this fight, read this summary from [Antonia Bertolino on the current testing research challenges](http://people.scs.carleton.ca/~jeanpier/sharedF14/Bertolino-state-of-artinSWTesting.pdf).
...
EDIT: Mariefred (from R/Programming) suggested this related link: [Testing and Checking refined](http://www.satisfice.com/blog/archives/856).
